...
- Node.js 與 npm（建議搭配 corepack enable 使用）。
- MariaDB 需開啟 utf8mb4。
- Redis 提供 session 與暫存鍵值。
...
oreply@example.com |
| Password_email | 寄件者信箱的應用程式密碼或 SMTP 密碼。 | xbcd efgh ijkl mnop |
...
- GET /api/yo_hualien
  - 讀取 loction_tour_hualien，回傳行動遊花蓮景點（站名、地址、座標）。
...
- GET /auth/line/callback
  - LINE 登入回呼端點，交換 access token、寫入使用者資料、發出簽名後的 app_session cookie。

- GET /logout
  - 清除 app_session cookie，導回預設前端網址。

- GET /me
  - 透過 app_session cookie 取得登入使用者資訊，若 Redis 或資料庫找不到對應 session 會回傳 401。
...
## 常見問題與建議
- **CORS**：後端預設允許 localhost、ngrok-free 及數個校內網段。若在其他網域部署需調整 allow_origin_regex。
- **Cookie 安全性**：FRONTEND_DEFAULT_URL 為 HTTPS 時，後端會將 app_session cookie 設為 Secure + SameSite=None；若為 HTTP，則使用 SameSite=Lax。
