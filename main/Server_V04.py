...
def update_user(user_name: str, user_update: UserUpdate, db: Session = Depends(get_db)):
    user = db.query(User).filter(User.username == user_name).first()
    if not user:
        raise HTTPException(status_code=404, detail="用戶不存在")
    ...
    for field_name in ['username', 'phone', 'email', 'line_id']:
        if field_name in update_data:
            ...
            exist = db.query(User).filter(getattr(User, field_name) == new_val, User.username != user_name).first()
            if exist:
                raise HTTPException(status_code=409, detail=f"{field_name} 已存在，請使用其他值")

    # 兼容：若更新帶入 reservation_status='None'，轉成 'no_reservation'
    if 'reservation_status' in update_data and update_data['reservation_status'] == 'None':
        update_data['reservation_status'] = 'no_reservation'
