    ...
    <div class="filters-section">
      ...
      <div class="filter-controls">
        ...
        <div class="filter-group">
          ...
          <select v-model="idSortOrder" @change="loadAdminUsers" class="page-size-select">
        ...
        <form @submit.prevent="showCreateModal ? createAdmin() : updateAdmin()" class="admin-form">
            ...
            v-if="
              ...
              (
                showEditModal && editingAdmin && (
                  // Super Admin 可改 Admin 或 Dispatcher 密碼；Admin 可改 Dispatcher 密碼
                  (isSuperAdmin && (editingAdmin.role_name === 'admin' || editingAdmin.role_name === 'dispatcher')) ||
                  (isAdmin && editingAdmin.role_name === 'dispatcher')
                )
              )
            "
            class="form-group"
          ...
          <div class="form-group">
            ...
            <div v-if="!isSuperAdmin" class="form-hint">
              高級管理員只能創建/修改 Dispatcher 角色帳號，且無法修改角色
            </div>
            ...
            <div v-else-if="showCreateModal && adminUsers.some(user => user.role_name === 'super_admin')" class="form-hint">
              系統已有 Super Admin，新增時不可再選 Super Admin
            </div>
          </div>
...
const canEditUser = computed(() => (admin: any) => {
  ...
  // Admin 只能編輯 Dispatcher
  if (isAdmin.value) {
    return admin.role_name === 'dispatcher'
  }
  // Dispatcher 不可編輯任何人
  return false
})
...
const canDeleteUser = computed(() => (admin: any) => {
  // Super Admin 可刪除除自己與其他 Super Admin 外的帳號
  if (isSuperAdmin.value) {
    if (admin.admin_id === currentUser.value?.user_id) return false
    if (admin.role_name === 'super_admin') return false
    return true
...
const availableRoles = computed(() => {
  // 角色資料格式：{ role_id, role_name, role_description }
  if (isSuperAdmin.value) {
    if (showCreateModal.value) {
      const hasSuperAdmin = adminUsers.value.some(user => user.role_name === 'super_admin')
      if (hasSuperAdmin) {
        // 已有 super_admin，建立新用戶時不可再選 super_admin
        return roles.value.filter((role: any) => role.role_name !== 'super_admin')
      }
      return roles.value
    } else {
      // 編輯模式
      if (editingAdmin.value && editingAdmin.value.admin_id === currentUser.value?.user_id) {
        return roles.value.filter((role: any) => role.role_id === editingAdmin.value.role_id)
      } else if (editingAdmin.value && editingAdmin.value.role_name === 'super_admin') {
        // 不允許對 super_admin 變更角色
        return roles.value.filter((role: any) => role.role_name === 'super_admin')
      }
      return roles.value
    }
  ...
  if (isAdmin.value) {
    // Admin 只能選 Dispatcher
    return roles.value.filter((role: any) => role.role_name === 'dispatcher')
  }
  // Dispatcher 無法建立/編輯
  return []
})
...
const isStatusSelectDisabled = computed(() => {
  ...
  // Admin 僅能編輯 Dispatcher 的狀態
  if (isAdmin.value) {
    if (!(showEditModal.value && editingAdmin.value)) return true
    return editingAdmin.value.role_name !== 'dispatcher'
  }
...
function resetForm() {
  formData.value = {
    username: '',
    password: '',
    // Super Admin：預設不選；Admin：自動帶入 Dispatcher 的 role_id
    role_id: isSuperAdmin.value
      ? ''
      : (roles.value.find((r: any) => (r.role_name || '').toLowerCase() === 'dispatcher')?.role_id || ''),
    status: 'active'
  }
}
...
function openCreateModal() {
  resetForm() // 重置表單
  // 再保險一次：若為 Admin 且角色列表已載入，強制指派 Dispatcher role_id
  if (!isSuperAdmin.value) {
    const disp = roles.value.find((r: any) => (r.role_name || '').toLowerCase() === 'dispatcher')
    if (disp) formData.value.role_id = disp.role_id
  }
